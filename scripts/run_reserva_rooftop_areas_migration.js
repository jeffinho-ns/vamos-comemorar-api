// Migração: Áreas do Reserva Rooftop em restaurant_areas (PostgreSQL)
...
async function runMigration() {
  ...
  try {
    ...
    const migrationPath = path.join(__dirname, '../migrations/add_reserva_rooftop_areas_postgresql.sql');
    if (!fs.existsSync(migrationPath)) throw new Error('Migração não encontrada: ' + migrationPath);
    const sql = fs.readFileSync(migrationPath, 'utf8');
    await pool.query(sql);
    console.log('Migração add_reserva_rooftop_areas executada com sucesso.');
  } catch (err) {
