...
async function insertPlace(client, { slug, name, organizationId }) {
  ...
  const hasVisible = await tableHasColumn(client, 'places', 'visible');
  const description = `Estabelecimento ${name}`;

  if (hasOrgCol && hasVisible) {
    const { rows } = await client.query(
      `INSERT INTO places (slug, name, email, description, status, visible, organization_id)
       VALUES ($1, $2, NULL, $3, 'active', TRUE, $4)
       RETURNING id`,
      [slug, name, description, organizationId],
    );
    return rows[0].id;
  }
  if (hasOrgCol) {
    const { rows } = await client.query(
      `INSERT INTO places (slug, name, email, description, status, organization_id)
  ...
  const { rows } = await client.query(
    `INSERT INTO places (slug, name, email, description, status)
    ...
    if (hasOrgCol) {
      await client.query(
        `INSERT INTO establishment_faq (establishment_id, topic, answer, category, is_active, organization_id, updated_at)
         SELECT $1::integer, $2::varchar, $3::text, $4::varchar, TRUE, $5::integer, now()
          WHERE NOT EXISTS (
            SELECT 1 FROM establishment_faq
             WHERE establishment_id = $1::integer AND topic = $2::varchar
          )`,
      ...
      await client.query(
        `INSERT INTO establishment_faq (establishment_id, topic, answer, category, is_active, updated_at)
         SELECT $1::integer, $2::varchar, $3::text, $4::varchar, TRUE, now()
          WHERE NOT EXISTS (
            SELECT 1 FROM establishment_faq
             WHERE establishment_id = $1::integer AND topic = $2::varchar
          )`,
