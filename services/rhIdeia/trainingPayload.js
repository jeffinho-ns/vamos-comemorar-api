...
const { str, optionalStr, parseBoolean, pickScope, parseEstablishmentId, resolveScopeEstablishment } = require('../../validators/rhIdeiaValidator');
...
function parseCreatePayload(body = {}) {
  ...
  const scope = pickScope(body.scope, 'organization');
  if (!scope.ok) return { ok: false, message: 'Escopo inválido. Use organization ou establishment.' };

  const establishmentId = resolveScopeEstablishment(scope.value, parseEstablishmentId(body.establishment_id));
  if (scope.value === 'establishment' && !establishmentId) {
    return { ok: false, message: 'establishment_id obrigatório quando scope=establishment.' };
  }
...
function parseUpdatePayload(body = {}) {
  ...
  if (body.content_body !== undefined) pushSet('content_body', optionalStr(body.content_body, 20000));
  ...
  if (body.is_mandatory !== undefined) pushSet('is_mandatory', parseBoolean(body.is_mandatory, true));
  if (body.is_active !== undefined) pushSet('is_active', parseBoolean(body.is_active, true));
  if (body.scope !== undefined) {
    const scope = pickScope(body.scope, null);
    if (!scope.ok || !scope.value) return { ok: false, message: 'Escopo inválido.' };
    pushSet('scope', scope.value);
    if (scope.value === 'organization') pushSet('establishment_id', null);
  }
  if (body.establishment_id !== undefined) {
    pushSet('establishment_id', parseEstablishmentId(body.establishment_id));
  }
